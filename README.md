...
## Features
- **Prayer times powered by location**: Uses Expo Location + Aladhan API to fetch daily salat times, highlights the current/next prayer, and handles Friday (Jummah) timing.
- **Dua collection**: Dozens of daily duas (see `constants/Duas.ts`) with Arabic, transliteration, translations, benefits, and sources.
...
## Tech stack
- Expo ~53.0.20 / React Native 0.79.5 / React 19.0.0 with Expo Router.
...
## Getting started
1) **Prerequisites**: Node 18+ and Expo tooling. (Install Expo CLI if needed: `npm install -g @expo/cli`). Enable Corepack for pnpm (`corepack enable`).
2) **Install**: `pnpm install --frozen-lockfile`
3) **Run** (pnpm uses the npm scripts in `package.json`):
   - Metro/dev server: `pnpm start`
   - Android emulator/device: `pnpm android`
...
## Testing
- Default script (watch mode via `jest --watchAll`): `pnpm test`
- One-time run: `pnpm test -- --watchAll=false`
